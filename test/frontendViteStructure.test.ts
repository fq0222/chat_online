...
test('前端工程必须使用 Vue3 + Vite 并放在 apps/web', () => {
  ...
  assert.match(frontEndSource, /openRoomEditor/);
  assert.match(frontEndSource, /closeRoomEditor/);
  assert.match(frontEndSource, /saveRoomSettings/);
  assert.match(frontEndSource, /method:\s*'PATCH'/);
  assert.match(frontEndSource, /remarkName:\s*roomEditDialog\.remarkName/);
  assert.match(frontEndSource, /welcomeMessage:\s*roomEditDialog\.welcomeMessage/);
  assert.match(frontEndSource, /class="room-edit-modal"/);
  assert.match(frontEndSource, /@open-room-editor="openRoomEditor"/);
  assert.match(frontEndSource, /@save-room-settings="saveRoomSettings"/);
  assert.doesNotMatch(frontEndSource, /class="room-welcome-editor"/);
  assert.doesNotMatch(frontEndSource, /@save-room-welcome-message/);
  ...
  assert.match(frontEndSource, /requestAnimationFrame\(\(\) => scheduleMessageTimelineBottomScroll/);
  assert.match(frontEndSource, /scrollToLatestReadMessage\(3\)/);
  assert.match(frontEndSource, /timeline\.style\.scrollBehavior = 'auto'/);
  ...
  assert.doesNotMatch(frontEndSource, /<input[^>]*type="file"[^>]*multiple/);
