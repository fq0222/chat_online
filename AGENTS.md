...
## 工作范围
只在当前项目目录下工作，有修改删除当前项目目录下文件的权限

## 工具使用
- 能使用 `rg` 命令完成搜索、定位、列文件的地方，优先使用 `rg`。
...
## 前端开发
- 前端必须使用 Vue3 + Vite 开发，工程目录固定为 `apps/web`
- 前端构建产物由 OpenResty 托管，禁止为了页面访问在 Express 后端中补静态资源路由
- PC 端和移动端聊天室后续分别适配；当前未明确要求时优先实现 PC 端
